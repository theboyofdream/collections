...
type bookmarkElementType = { title: string, href: string, element: ReactElement<any, any> }
type bookmarkElements = { [key: string]: bookmarkElementType }
export default function Home() {
  ...
  const
    ...
    [bookmarkElements, setBookmarkElements] = useState<bookmarkElements>({}),
  ...
  useEffect(() => {
    // setSearchBoxElem(document.getElementById("search-box") as HTMLInputElement)
    ...
    let _bookmarkElements: bookmarkElements = {}

    arrOfLinks.map(({ title, href }, index) => {
      _bookmarkElements[href] = {
        title,
        href,
        element: (<a
          key={index}
    ...
    setBookmarkElements(_bookmarkElements)
  }, [])
        ...
        <main>
          {
            filteredArrOfBookmarks.map(({ title, href }, index) => {
              // console.log(bookmarkElements[href])
              // return <></>
              // return bookmarkElements[href]?.element
              // (bookmarkElements[href].element)
              // }
              return <a
                key={index}
                className='link'
